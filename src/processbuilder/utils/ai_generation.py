...
def generate_step_description(openai_client, process_name: str, step_id: str, predecessor_id: Optional[str] = None, 
    ...
    try:
        ...
        if len(words) > 50:
            ...
            prompt += "\nThe description was too short. Please provide a more detailed description between 30-50 words."
            response = openai_client.chat.completions.create(
                model="gpt-4-turbo-preview",
                messages=[
                    {"role": "system", "content": "You are a business process expert. Create clear, concise step descriptions that follow best practices."},
...
def generate_step_with_ai(
    prompt: str,
    existing_steps: List[ProcessStep],
    api_key: Optional[str] = None
) -> ProcessStep:
    """Generate a process step using AI.
    
    Args:
        prompt: The prompt describing the step to generate
        existing_steps: List of existing steps to check for uniqueness
        api_key: Optional OpenAI API key
        
    Returns:
        A new ProcessStep generated by AI
    """
    try:
        if api_key:
        ...
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "You are a process design expert. Generate a clear, concise process step."},
                {"role": "user", "content": prompt}
            ]
        )
        ...
        # Parse the response into step components
        lines = content.split("\n")
        description = lines[0] if lines else "AI-generated step"
        decision = lines[1] if len(lines) > 1 else "Is this step complete?"
        success_outcome = lines[2] if len(lines) > 2 else "Proceed to next step"
        failure_outcome = lines[3] if len(lines) > 3 else "Retry or escalate"
        
        # Create a unique step ID
        step_id = f"ai_step_{len(existing_steps) + 1}"
        ...
        log.error(f"Error generating step with AI: {str(e)}")
        raise
...
def generate_note_with_ai(
    ...
    """Generate a process note using AI.
    
    Args:
        step: The ProcessStep to generate a note for
        prompt: Optional prompt to guide note generation
        api_key: Optional OpenAI API key
        
    Returns:
        A new ProcessNote generated by AI
    """
    try:
        if api_key:
            openai.api_key = api_key
        ...
        if not prompt:
            prompt = f"Generate a helpful note for the step: {step.description}"
            
        # Generate note content
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "You are a process documentation expert. Generate a clear, helpful note."},
                {"role": "user", "content": prompt}
            ]
        )
        ...
        # Create note ID
        note_id = f"ai_note_{step.step_id}"
        ...
        log.error(f"Error generating note with AI: {str(e)}")
        raise
